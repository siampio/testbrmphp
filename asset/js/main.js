(function (jQuery) {
   ...
   $('#btn-edit').click(function(){
    $('#btn-edit').hide()
    $('.Mensaje').hide();
    $('.form-email .text-email').hide();
    
    var cadena = $('.Mensaje').html();
    ...
    var promise = new Promise(function(resolve, reject){
        ...
        sin_salto = sin_salto.replace("/<[^>]*>?/g","");
        
        resolve(sin_salto);
    });
    promise.then(function(result){
        var from = $('#txt-from').html();
    var asunto = $('#txt-asunto').html();
    $('#text-from').val(from);
    $('#text-asunto').val(asunto);

    $('#text-areaEmail').val(texto.trim());
    $('#text-areaEmail').show();
    $('.btn-edits').css("display","inline-block");
    $('.form-email input').css("display","block");
    });
...
$('#btn-save').click(function(){
    ...
    var html = $('#text-areaEmail').val();
    var from = $('#text-from').val();
    ...
    if(from ==="" || asunto==="" || html===""){
        ...
        if(caract.test(from) == false){
            alert('El campo correo es invalido');
        }else{
            html="<p>"+html+"</p>";
            $('#txt-from').text(from);
            $('#txt-asunto').text(asunto);
            $('.Mensaje').html(html);

            $('#text-areaEmail').hide();
            $('.btn-edits').hide();
            $('.form-email input').hide();
            $('.Mensaje').show();
            $('#btn-edit').show();
            $('.form-email .text-email').show();
        }
...
$('#btn-edit-sms').click(function(){
    ...
    $('#btn-edit-sms').hide();
    $('.Mensaje').hide();
    $('.btn-edits-sms').css("display","inline-block");
    $('#text-areaSMS').val(texto.trim());
...
$('#btn-save-sms').click(function(){
    ...
    if(msj ===""){
        ...
        if(msj.length >160){
            alert('Supera el maximo de caracteres de 160');
        }else{
            $('#txt-msj-sms').text(msj);
            $('#btn-edit-sms').show();		
            $('.form-sms .txt-sms').show();
            $('.btn-edits-sms').hide();
            $('.form-sms input').hide();	
        }
